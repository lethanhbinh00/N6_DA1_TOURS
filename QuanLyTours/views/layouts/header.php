...
<head>
    ...
    <style>
        ...
        /* Sidebar Styling */
        .sidebar { width: 260px; height: 100vh; position: fixed; background: var(--sidebar-bg); color: var(--sidebar-color); overflow-y: auto; z-index: 1000; transition: all 0.3s; }
        ...
        .nav-link { color: #bdc3c7; padding: 12px 20px; display: flex; justify-content: space-between; align-items: center; text-decoration: none; cursor: pointer; }
        .nav-link:hover, .nav-link.active { color: #fff; background: var(--active-bg); }
        .submenu { background: #233342; display: none; } /* Mặc định ẩn */
        .submenu.show { display: block; } /* Class để hiện menu */
...
<div class="sidebar">
    ...
    <?php $act = $_GET['action'] ?? ''; // Lấy action hiện tại để active menu ?>
    ...
    <ul class="nav flex-column">
        ...
        <li class="nav-item">
            <a class="nav-link" onclick="toggleMenu('menu-tour')">
                <div class="d-flex align-items-center"><i class="fas fa-suitcase me-2"></i> Quản lý Tour</div>
                <i class="fas fa-chevron-down small"></i>
            </a>
            <div id="menu-tour" class="submenu <?= (strpos($act, 'store')!==false || strpos($act, 'index')!==false)?'show':'' ?>">
                <a href="index.php?action=index" class="nav-link">Danh sách Tour</a>
                <a href="#" class="nav-link">Tour Theo Yêu Cầu</a>
                <a href="#" class="nav-link">Quản lý Phiên bản</a>
            </div>
        </li>

        <li class="nav-item">
            <a class="nav-link" onclick="toggleMenu('menu-booking')">
                <div class="d-flex align-items-center"><i class="fas fa-shopping-cart me-2"></i> Booking & Sales</div>
                <i class="fas fa-chevron-down small"></i>
            </a>
            <div id="menu-booking" class="submenu <?= (strpos($act, 'booking')!==false)?'show':'' ?>">
        ...
        <li class="nav-item">
            <a class="nav-link" onclick="toggleMenu('menu-ops')">
        ...
        <li class="nav-item"><a href="#" class="nav-link"><div class="d-flex align-items-center"><i class="fas fa-file-invoice-dollar me-2"></i> Tài chính & NCC</div></a></li>
